...
class RegisterManualApp(QMainWindow):
    ...
    def create_excel_xml(self, excel_df, register_data, sgml_topic_id, sgml_filename):
        ...
        QMessageBox.information(self, "Success", f"Excel XML created successfully: {output_xml}")
        return output_xml
    ...
    def create_sgml_xml(self, register_data, sgml_topic_id, output_xml_base_name):
        ...
        if register_data:
            for reg in register_data:
                ...
                if reg['addr_table']:
                    addr_table = reg['addr_table']
                    if len(addr_table) >= 3:
                        etree.SubElement(reg_elem, "p").text = addr_table[0][0]
                        etree.SubElement(reg_elem, "p").text = addr_table[1][0]
                        etree.SubElement(reg_elem, "p").text = addr_table[2][0]
        ...
        tree = etree.ElementTree(root)
        tree.write(output_xml, pretty_print=True, xml_declaration=True, encoding="UTF-8")
        QMessageBox.information(self, "Success", f"SGML XML created successfully: {output_xml}")
        return output_xml
